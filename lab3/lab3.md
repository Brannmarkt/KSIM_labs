...
Симуляція, побудована на параметрах, отриманих за допомогою рандомного пошуку, демонструє досить нестійку та різко змінну динаміку популяцій. Вівці спочатку зростають, але швидко падають, а вовки, хоч і проходять свій типовий цикл, не досягають критично високих значень — що свідчить про те, що параметри репродукції, приросту енергії та регенерації трави виявилися випадковим підбором не оптимізованими. Трава в цій моделі відновлюється повільно, тому обидві популяції функціонують на межі ресурсного голодування. У підсумку така симуляція відображає середній, нестабільний сценарій, де екосистема вразлива та не здатна підтримувати значну чисельність вовків або овець протягом тривалого часу.
...
Популяція вовків зростає до значно більших значень — понад тисячу особин — що напряму підтверджує результати Behavioral Search, де генетичний алгоритм знайшов набір параметрів, що забезпечують значно кращу «фітнес» метрику. Вівці також проходять фазу стрімкого зростання, що підтримує високий рівень доступної їжі для вовків. Регрес популяцій у фінальній частині симуляції залишається нормальним результатом моделі хижак-жертва, проте масштаб і тривалість піків популяцій стають суттєво більшими, ніж у рандомному варіанті. Стан трави у цій симуляції також значно стабільніший — це пов’язано з тим, що генетичний пошук підібрав такі параметри регенерації та енергетичних витрат, які дозволяють траві відновлюватися швидше і підтримувати довготривалі коливання системи.

Таким чином, генетичний алгоритм не просто дав кращу числову оцінку фітнесу, а й на практиці привів до створення стабільнішої та «продуктивнішої» в термінах популяцій екосистеми. Симуляція за параметрами генетичного пошуку показує високі піки чисельності, тривале існування популяцій і правильну амплітуду коливань, що повністю узгоджується з кривими зростання фітнесу під час Behavioral Search. 
У підсумку можна сказати, що результати обох симуляцій переконливо демонструють: генетичний пошук не лише знайшов параметри з вищою метрикою, але й створив модель поведінки системи, яка суттєво відрізняється якісно — з більш вираженими та стійкими популяційними циклами, тоді як рандомний пошук лише випадково знаходив середні, часто нестійкі рішення, які не здатні забезпечити довготривале існування великих популяцій.
